...
const categoryIcons: Record<string, keyof typeof Ionicons.glyphMap> = {
  cleaning: "sparkles",
  plumb: "construct", // Changed 'plumbing' to match typical id prefixes if needed, adjust key to match your data ids
  plumbing: "construct",
  electrical: "flash",
...
export function HomeScreen() {
  ...
  const { currentLocation, getCartCount } = useAppStore()
  ...
  const handleCategoryPress = (id: string) => {
    // If you haven't built the category page yet, this might error. 
    // For now, let's just log it or push to search with a query
    console.log("Navigating to category:", id);
    router.push(`/category/${id}` as any);
  }

  const handleServicePress = (id: string) => {
    // This matches your file: app/service/[id].tsx
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontalScroll}>
            {featuredServices.map((service) => (
              <TouchableOpacity
                ...
                // ✅ FIXED: Explicitly calling router via handler
                onPress={() => handleServicePress(service.id)}
              >
                <View style={styles.featuredImageContainer}>
                  <Image source={{ uri: serviceImages[service.id] || service.image }} style={styles.featuredImage} />
                  {service.originalPrice && (
                    <View style={styles.discountBadge}>
                      <Text style={styles.discountText}>{Math.round((1 - service.price / service.originalPrice) * 100)}% OFF</Text>
                    </View>
                  )}
                </View>
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: Colors.primary, paddingTop: Platform.OS === 'android' ? (StatusBar.currentHeight || 0) + 20 : 60, paddingHorizontal: 20, paddingBottom: 24, borderBottomLeftRadius: 24, borderBottomRightRadius: 24 },
  ...
  searchBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.15)', padding: 12, borderRadius: 12 },
  ...
  popularImage: { width: 70, height: 70, borderRadius: 8, resizeMode: 'cover', backgroundColor: '#f0f0f0' },
  popularContent: { flex: 1, marginLeft: 12 },
  popularName: { fontSize: 15, fontWeight: '600', color: Colors.text, marginBottom: 4 },
  bottomNav: {
    position: 'absolute', bottom: 0, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center',
    backgroundColor: Colors.white, paddingVertical: 12, borderTopWidth: 1, borderTopColor: Colors.border,
    paddingBottom: Platform.OS === 'ios' ? 24 : 12, elevation: 20, shadowColor: "#000", shadowOffset: { width: 0, height: -4 },
